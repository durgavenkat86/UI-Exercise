...
const Dropdown: React.FC<DropdownProps> = (props: DropdownProps) => {
  ...
  const [selectedOption, setSelectedOption] = useState<IDropdownOption>({ id: '', name: '' });
  const [searchText, setSearchText] = useState<string>('');

  const filteredDropdownOptions: IDropdownOption[] =
    isEmpty(searchText)
      ? options
  ...
  const renderFilteredMenu = (): JSX.Element[] => {
    return (
      filteredDropdownOptions.map((item) => (
        <Combobox.Option
          key={item.id}
          className={({ active }) => classNames('relative', 'p-2', 'rounded-md', 'cursor-pointer', { 'bg-blue-600': active, 'text-white': active, 'text-gray-900': !active })}
          value={item}>
          <div className="flex justify-between">
            <div className='flex'>
              <img src={IMAGE_MAP[item.image || item.id]} className="w-5 h-5 mr-4 rounded-sm" />
              <span className="block truncate">{item.name}</span>
            </div>
            {
              (selectedOption.id === item.id)
              && <span className="h-5 w-5" aria-hidden="true">{icon.check}</span>
            }
          </div>
        </Combobox.Option>
      ))
    ...
    <div>
      <Combobox value={selectedOption} onChange={handleOnSelect}>
        <div className="relative mt-1">
          {(isEmpty(searchText) && isEmpty(selectedOption.name)) && <img src={searchLogo} className="w-4 h-4 text-gray-500 absolute mt-2.5 ml-2 focus:invisible" />}
          <Combobox.Input
