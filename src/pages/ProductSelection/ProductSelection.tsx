...
// Common Components
import { Button, Dropdown, AddProduct, SelectedProduct } from '../../components';
...
const ProductSelection: React.FC = () => {
  ...
  const onSelectProduct = (product: IProduct): void => {
    ...
    if (selectedProducts.length < MAX_PRODUCTS_SIZE) {
      setSelectedProducts((products: IProduct[]) => uniqBy([...products, product], 'id'));
      setShowNotification(false);
    }
  }

  const onProductRemove = (product: IProduct): void => {
    setSelectedProducts(selectedProducts.filter((item: IProduct) => item.id !== product.id));
  }

  const onClickNext = (): void => {
    // Send selected products to API
    ProductsService.saveProducts(selectedProducts).then((response: any) => {
  ...
  const renderAddProductsMenu = (): JSX.Element => {
    return (
      <div className="w-96 p-5">
        ...
        {showNotification && <p className="text-red-500 text-sm">{t('ONLY_4_PRODUCTS_ALLOWED')}</p>}
        <Dropdown options={products} onSelect={onSelectProduct} />
        <div className="mt-8">
          <Button type="button" label="Next" onClick={onClickNext}
        ...
        <div className="grid gap-6 grid-cols-2">
          {
            selectedProducts.map((product: IProduct) =>
              <SelectedProduct key={product.id} product={product} onRemove={onProductRemove} />)
          }
          {
            products.slice(0, MAX_PRODUCTS_SIZE - selectedProducts.length)
              .map((product, index) => <AddProduct key={index} />)
          }
        </div>
        <p className="text-center mt-5 text-gray-500">
          {
            selectedProducts.length === 1
              ...
              : t('PRODUCTS_ADDED_COUNT_PLURAL', { count: selectedProducts.length })
          }
        </p>
      </div>
    )
